...
Existing HTML section anchors are retained. The repair splits existing text nodes; it never invents missing legal wording or copies wording across representations. Ambiguous or absent matches produce unresolved findings. A heading is promoted only if an entire remaining HTML text node matches one observed text heading in that chapter. Heading detection remains a heuristic, recorded in the report for review; it does not establish the legal level of every heading.
